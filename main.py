...
class ConversationSession:
    def __init__(self):
        self.id = str(uuid.uuid4())
        self.state = "greeting"  # greeting, name, service, datetime, confirmation
        self.user_name = None
...
def extract_datetime(text: str) -> tuple:
    """
    Extract date and time using dateutil.parser, returns date YYYY-MM-DD and time hh:mm AM/PM
    """
    text = text.strip()
    today = datetime.now()
    try:
        ...
        time_str = dt.strftime("%I:%M %p")  # AM/PM format
